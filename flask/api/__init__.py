"""
Welcome to the documentation for the FlaskStack API!
...
[project's source code](https://github.com/jimmygizmo/flaskstack/flask) using
the [APIFairy](https://github.com/miguelgrinberg/apifairy) Flask extension.

## Introduction

FlaskStack-API is an easy to use web API for creating REST APIS, especially
as a backend to The Nucleus Stack. It is an ideal
project to use when learning a front end framework, as it provides a fully
implemented back end that you can integrate against. And of course, this is
an example of the best way to implement a REST backend. The Nucleus Stack
comes with a GraphQL backend, but it is easy to use a leading-edge REST backend
instead of or alongside of the default backend. Personally, I like having both
styles of data layers behind an application and many complex applications are
well suited to having both a GraphQL as well as a REST/SQL backend.

The FlaskStack REST API provides all the base features required to implement a
high quality, production-ready REST API project with common features.
The limited examples are for a blogging application, but the basic features and
schema are immediately extensible into any kind of application you want to build.
...
If you are running FlaskStack API yourself while developing your front end,
...
| `MAIL_DEFAULT_SENDER` | `donotreply@flaskstack.example.com` | The default sender to use for emails. |
